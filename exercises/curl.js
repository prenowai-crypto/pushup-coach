...
const VALIDITY={
  UPPER_ARM_INVALID:40,  // invalidate rep when humerus rises clearly away from torso
  UPPER_ARM_REARM:30,    // must return below this to become valid again
  INVALID_FRAMES:4
};
const TECH={
  // V6.5: geometria torso-median.
  // Soglie volutamente conservative finché raccogliamo dati reali con questa nuova misura.
  OUTWARD_DELTA_WARN:.16,
  OUTWARD_RATIO_WARN:.55,
  VOICE_COOLDOWN:4500
};
...
function newArm(name,sh,el,wr){return{name,sh,el,wr,state:"GET_READY",smooth:null,topFrames:0,extFrames:0,repStart:0,reps:0,lastRepMs:0,lastRaw:null,lastVis:0,invalidFrames:0,invalidLatched:false,lastInvalidVoice:0,tech:{baseline:null,repBase:null,repMax:null,last:null,outBase:null,outMax:null,medianRep:null}}}
...
function torsoMedianMetrics(lm,a){
  if(!lm)return null;
  const ls=lm[11],rs=lm[12],lh=lm[23],rh=lm[24];
  const elbow=lm[a.el],hand=lm[a.wr],hip=lm[a.name==="SX"?23:24];
  if(!ls||!rs||!lh||!rh||!elbow||!hand||!hip)return null;

  const shoulderMid=midpoint(ls,rs),hipMid=midpoint(lh,rh);
  const shoulderWidth=Math.hypot(ls.x-rs.x,ls.y-rs.y);
  if(!shoulderMid||!hipMid||shoulderWidth<1e-6)return null;

  const elbowMedian=pointLineDistance(elbow,shoulderMid,hipMid)/shoulderWidth;
  const hipMedian=pointLineDistance(hip,shoulderMid,hipMid)/shoulderWidth;
  const handHip=Math.hypot(hand.x-hip.x,hand.y-hip.y)/shoulderWidth;

  // Positive => elbow is farther out from spinal/torso median than its own hip.
  const outward=elbowMedian-hipMedian;
  return {elbowMedian,hipMedian,handHip,outward};
}
function analyzeArm(lm,a,elbowToTorso,shoulderAngle){
  ...
  const ang=a.smooth,now=performance.now();let repEvent=false;const median=torsoMedianMetrics(lm,a);
  ...
  // Stricter Curl validity gate with hysteresis.
  // Once invalid, the arm must come back clearly alongside the torso before re-arming.
  if(a.invalidLatched){
    if(Number.isFinite(shoulderAngle)&&shoulderAngle<=VALIDITY.UPPER_ARM_REARM){
      a.invalidLatched=false;a.invalidFrames=0;
    }else{
  ...
  const invalidPosition=Number.isFinite(shoulderAngle)&&shoulderAngle>=VALIDITY.UPPER_ARM_INVALID;
  a.invalidFrames=invalidPosition?a.invalidFrames+1:0;
  if(a.invalidFrames>=VALIDITY.INVALID_FRAMES){
    a.invalidLatched=true;
    a.state="GET_READY";a.topFrames=0;a.extFrames=0;a.repStart=0;
    return {ok:true,arm:a,ang,raw,vis,repEvent:false,valid:false,shoulderAngle};
  }
  ...
  if(["CURL_UP","TOP","LOWER"].includes(a.state)){
    ...
    if(Number.isFinite(median?.outward))
      a.tech.outMax=a.tech.outMax==null?median.outward:Math.max(a.tech.outMax,median.outward);
  }
  ...
  if(a.state==="GET_READY"){
    ...
    if(ang<CFG.START_CURL){
      ...
      a.tech.outBase=Number.isFinite(median?.outward)?median.outward:null;
      a.tech.outMax=Number.isFinite(median?.outward)?median.outward:null;
      a.state="CURL_UP"
    }
    ...
    if(ang>CFG.EXTENDED){
      if(++a.extFrames>=CFG.EXT_FRAMES){
        ...
        if(ms>=CFG.MIN_REP&&ms<=CFG.MAX_REP){
          ...
          if(Number.isFinite(a.tech.outBase)&&Number.isFinite(a.tech.outMax)&&median){
            a.tech.medianRep={
              base:a.tech.outBase,max:a.tech.outMax,delta:a.tech.outMax-a.tech.outBase,
              elbowMedian:median.elbowMedian,hipMedian:median.hipMedian,handHip:median.handHip
            };
          }else a.tech.medianRep=null;
        }
...
function validateTechnique(a){
  const m=a.tech.medianRep;
  if(!m||![m.base,m.max,m.delta].every(Number.isFinite))
    return {status:"UNAVAILABLE",error:null};

  // Elbow error = how much farther outward the elbow is than the same-side hip,
  // and how much that outward offset grows during the rep.
  const badDrift=m.delta>=TECH.OUTWARD_DELTA_WARN;
  const badOutward=m.max>=TECH.OUTWARD_RATIO_WARN;

  if(!(badDrift||badOutward))
    return {status:"OK",error:null,median:m};

  return {
    status:"ERROR",error:"ELBOW_AWAY",median:m,
    reason:badDrift?"OUTWARD_DRIFT":"OUTWARD_POSITION"
  };
}
function emitTechnique(side,v){
  ...
  window.dispatchEvent(new CustomEvent("exercise-technique",{detail:{
    ...
    speak,metrics:{median:v.median,reason:v.reason}
  }}));
  ...
  analyze(frame){
    ...
    for(const x of [L,R])if(x.repEvent){
      ...
      window.dispatchEvent(new CustomEvent("exercise-rep",{detail:{
        ...
        technique:{elbowToTorso:x.arm.tech.last,torsoMedian:x.arm.tech.medianRep,validation}
      }}));
    ...
    return{
      ...
      debug:`CURL V6.6 STRICT-GATE | SX ${fmt(L)} raw=${Number.isFinite(arms.left.lastRaw)?Math.round(arms.left.lastRaw):"--"} vis=${Math.round(arms.left.lastVis*100)}% ${arms.left.state} rep=${arms.left.reps} | DX ${fmt(R)} raw=${Number.isFinite(arms.right.lastRaw)?Math.round(arms.right.lastRaw):"--"} vis=${Math.round(arms.right.lastVis*100)}% ${arms.right.state} rep=${arms.right.reps}`
    };
  }
};
