...
function newArm(name,sh,el,wr){return{name,sh,el,wr,state:"GET_READY",smooth:null,topFrames:0,extFrames:0,repStart:0,reps:0,lastRepMs:0,lastRaw:null,lastVis:0,invalidFrames:0,lastInvalidVoice:0,tech:{baseline:null,repBase:null,repMax:null,last:null,outBase:null,outMax:null,medianRep:null}}}
...
const CurlExercise={
  ...
  analyze(frame){
    ...
    return{
      ...
      debug:`CURL V6.5 TORSO-MEDIAN | SX ${fmt(L)} raw=${Number.isFinite(arms.left.lastRaw)?Math.round(arms.left.lastRaw):"--"} vis=${Math.round(arms.left.lastVis*100)}% ${arms.left.state} rep=${arms.left.reps} | DX ${fmt(R)} raw=${Number.isFinite(arms.right.lastRaw)?Math.round(arms.right.lastRaw):"--"} vis=${Math.round(arms.right.lastVis*100)}% ${arms.right.state} rep=${arms.right.reps}`
    };
  }
};
